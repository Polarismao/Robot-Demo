...
void I2C_Mode_Config(void)
{
    ...
    I2C_InitStructure.I2C_OwnAddress1 = I2C_OWN_ADDRESS7;                        // I2C 自身地址
    ...
    I2C_Init(PCA9685_I2C1, &I2C_InitStructure);                                  // 写入配置
    I2C_Cmd(PCA9685_I2C1, ENABLE);                                               // 使能 I2C
}
...
void I2C_ByteWrite(unsigned char uc_data, unsigned char uc_writeAddr)
{
    ...
    I2C_GenerateSTART(PCA9685_I2C1, ENABLE);         //产生 I2C 起始信号

    I2CTimeout = I2CT_FLAG_TIMEOUT;                  //设置超时等待时间
    while (!I2C_CheckEvent(PCA9685_I2C1, I2C_EVENT_MASTER_MODE_SELECT))  //检测 EV5 事件并清除标志
    {
        if ((I2CTimeout--) == 0)   
        {
            return;
    ...
    I2C_Send7bitAddress(PCA9685_I2C1, PCA9685_ADDR, I2C_Direction_Transmitter);  //发送 PCA9685 设备地址

    I2CTimeout = I2CT_FLAG_TIMEOUT;
    while (!I2C_CheckEvent(PCA9685_I2C1, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))  //检测 EV6 事件并清除标志
    {
        if ((I2CTimeout--) == 0)
        {
            return;
    ...
    I2C_SendData(PCA9685_I2C1, uc_writeAddr);      //发送要写入的 PCA9685 内部地址 (即 PCA9685 内部存储器的地址)

    I2CTimeout = I2CT_FLAG_TIMEOUT;
    while (!I2C_CheckEvent(PCA9685_I2C1, I2C_EVENT_MASTER_BYTE_TRANSMITTED))    //检测 EV8 事件并清除标志
    {
        if ((I2CTimeout--) == 0)
        {
            return;
    ...
    I2C_SendData(PCA9685_I2C1, uc_data);       //发送一字节要写入的数据
    ...
    I2C_GenerateSTOP(PCA9685_I2C1, ENABLE);      //发送停止信号
	return;
}
